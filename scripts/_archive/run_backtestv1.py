...
def main(args):
    ...
    # quick sanity: are we ever tradable?
    tradable = df["t_local"].apply(risk.can_trade_now)
    print("[SANITY] bars:", len(df),
        "tradable:", int(tradable.sum()),
        "tradable_pct:", float(tradable.mean()) if len(df) else 0.0)
    print("[SANITY] t_local min/max:", df["t_local"].iloc[0], df["t_local"].iloc[-1])
    print("[SANITY] dates:", df["date"].min(), "->", df["date"].max())
    ...
    print("[DBG] session_windows =", windows)
    ...
    for i, (_, bar) in enumerate(df.iterrows()):
        ...
        if not risk.can_trade_now(bar["t_local"]):
            if i % 500 == 0:
                print(f"[DBG] {bar['date']} {bar['t_local']} can_trade={risk.can_trade_now(bar['t_local'])} halted={risk.halted}")
        ...
        # new signal
        if i % 500 == 0:
            print(f"[DBG] signal_check at {bar['date']} {bar['t_local']} close={bar['close']}")
        # new signal
...
if __name__ == "__main__":
    ...
    ap.add_argument("--universe", default=None, help="Universe yaml name (e.g., sp500_fractional)")
